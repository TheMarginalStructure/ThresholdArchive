...
核心特性：
  1. approxPolyDP几何简化（epsilon控制精度，越小越保真）
  2. 比例正交化：基于 dx/dy 比例强制水平/垂直边对齐
  3. 拐角修正：直角拐角处强制坐标一致，保证UI/文字横平竖直
  4. nonzero填充规则：正确处理多层嵌套轮廓（孔洞中的孔洞）
  5. 所有坐标严格整数，确保SVG文件简洁精确
  6. 不强行拟合曲线，避免椭圆检测错误导致的变形

依赖安装：
    pip install numpy pillow opencv-python

基本用法：
    python bitmap_to_vector.py input.png                    # 自动输出 input.svg
    python bitmap_to_vector.py input.png output.svg         # 指定输出文件名
    python bitmap_to_vector.py input.png --invert --eps 0.5 # 使用选项参数
...
场景推荐：
  极致精细:  --eps 0.5  （圆弧几乎完美，点数多，文件稍大）
  平衡模式:  --eps 1.0  （推荐，肉眼无损，文件大小适中）
  强压缩:    --eps 3.0  （文件最小，略有锯齿）
"""
...
def bitmap_to_vector(image_path, output_svg_path,
    ...
    if path_segments:
        SubElement(svg, 'path', {
            ...
            'fill': fill_color,
            'fill-rule': 'nonzero',  # nonzero正确处理多层嵌套
            'stroke': 'none'
        })
...
def main():
    ...
    if len(sys.argv) < 2:
        print('用法: python bitmap_to_vector.py <输入图片> [输出.svg] [选项]')
        print()
        print('选项:')
        print('  --invert         提取黑色前景（默认提取白色前景）')
        print('  --eps N          approxPolyDP容差像素（默认1.0）')
        print('  --snap N         正交吸附容差像素（默认2.0）')
        print('  --threshold N    二值化阈值（默认128，-1=Otsu自动阈值）')
        print()
        print('场景推荐:')
        print('  极致精细:  --eps 0.5  （圆弧几乎完美，点数多，文件稍大）')
        print('  平衡模式:  --eps 1.0  （推荐，肉眼无损，文件大小适中）')
        print('  强压缩:    --eps 3.0  （文件最小，略有锯齿）')
        print()
        print('示例:')
        print('  python bitmap_to_vector.py icon.png')
        print('  python bitmap_to_vector.py icon.png icon.svg')
        print('  python bitmap_to_vector.py icon.png --invert --eps 0.5')
        sys.exit(1)
